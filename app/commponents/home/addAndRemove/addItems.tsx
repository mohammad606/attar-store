...
import {data, dataInOut, dataStoryType, storyObjType,items,qtn} from "@/app/type/typeDataFirebase";


const AddItems = ({InputLimit,dataStore,setPending,startTransition,setShowAdded}
:
{
 InputLimit:dataInOut,
 ...
 setShowAdded:any
    ...
    const allQtn:number = arrayOfQtn?arrayOfQtn.reduce((accumulator:number, currentValue:number) => accumulator + currentValue, 0):0;
    ...
    const HandleAddToDatabase = (e:any)=>{
        ...
        sendInputData(dataSend, `${coc}/Input and output data/Input/${dataSend.id}`).then((r:void) =>r)
                            ...
                            <div className='w-full bg-[#1f2937] ' key={index}>
                                ...
                                <table className='w-full'>
                                    ...
                                    <tr className='flex flex-col w-1/2 '>
                                        {items?.map((d:string,indexD:number)=>(
                                            <td key={indexD}  className='mb-2 border-b-2 pl-4'>{d}</td>
                                        ))
                                        }
                                    </tr>
                                    <tr className='flex flex-col w-1/2'>
                                        {qtn?.map((f:number,indexF:number)=>(
