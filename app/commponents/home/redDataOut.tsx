...
const RedDataOut = ({dataOutLimit,dataStore,isMutating,setPending,startTransition}
    :
    {
    ...
    let rot = useRouter()
    const removeUndLimit :dataInOut= RemoveUndefinedFromArray(dataOutLimit)
    ...
    const handleDelete = (e:data,items:items,qtn:qtn)=>{
        Swal.fire({
            ...
            if (result.isConfirmed) {
                HandleDeletOrder(removeUndLimit,removeUndStore,e.id,items,qtn,`Input and output data/Output`,coc,'order')
                setPending(true);
        ...
        <div className='relative z-2 w-1/2 flex flex-col  border-r-2  border-t-2 msm:w-full msm:border-2'>
            ...
            <div className='w-full h-auto flex  border-gray-900 border-r-2'>
                <div className='mx-2 flex flex-col-reverse w-full'>
                    {isMutating?<p className='p-5'>update....</p>:removeUndLimit.length >= 1?removeUndLimit?.map((e,index)=>{
                            const items = e.items
                            const qtn = e.qtn

                            return(
                                <div key={index} className='w-full bg-[#1f2937] flex flex-col my-2 py-2 rounded-2xl' id={`${e?.id}`}>
                                    ...
                                    <div className='w-full flex flex-row justify-around '>
                                        <div>
                                            ...
                                            <button type="button"
                                                ...
                                                <Link href={`editOrder/${e.id}/output`}>
                                    ...
                                    <table>
                                        ...
                                        <tr className='flex flex-col w-1/2 smm:text-[13px]'>
                                            {items?.map((d,indexD)=>(
                                                <td key={indexD} className='mb-2 border-b-2 pl-4'>{d}</td>
                                            ))
                                            }
                                        </tr>
                                        <tr className='flex flex-col w-1/2 smm:text-[13px]'>
                                            {qtn?.map((f,indexF)=>(
                                                <td key={indexF} className='mb-2 border-b-2 w-full text-center'>{f}</td>
                                            ))
                                            }
                                        </tr>
                                        </tbody>
